                ...
                if (instanceSelections.length == 0) {
                    ...
                    if (titleInstance.type == "INSTANCE") {
                        ...
                        for (const instanceSelection of instanceSelections) {
                            ...
                            yield loadNodeFonts([numberChild, titleChild]);
                    ...
                    if (declareInstance.type == "INSTANCE") {
                        ...
                        yield loadNodeFonts([declareChild]);
...
function frameNamer() {
    var selections = figma.currentPage.selection;
    for (const selection of selections) {
        if (selection.type == "FRAME" || selection.type == "GROUP") {
    ...
    // for (const selection of selections) {
    //   if (selection.type == "FRAME") {
    //     var backgroudNode = selection.findOne(n => n.type == "INSTANCE" && n.mainComponent.name === "交互文档页面")
    ...
    if (lines.length == 0) {
        ...
        for (const line of lines) {
            if (line.type == "LINE") {
                ...
                var newVector = createVectorByPoints([startPoint,
...
function translate() {
    return __awaiter(this, void 0, void 0, function* () {
        yield figma.loadFontAsync({ family: "OPPOSans", style: "Regular" });
    });
}
